...
class CollapsibleQuestion(QWidget):
    """Card-like collapsible editor for a single question."""
    def __init__(self, qid, category, role, text, score=0.0, notes="", parent=None):
        ...
        # Header
        self.header_btn = QPushButton(f"[{self.category}] {self.role or ''} | Score: {self.score:.2f}")
        ...
        # Question editor
        lbl_q = QLabel("Edit Question")
        body_layout.addWidget(lbl_q)
        self.text_edit = QTextEdit()
        self.text_edit.setPlainText(self.text_value)
        ...
        # Notes editor
        lbl_n = QLabel("Facilitator Notes (optional)")
        body_layout.addWidget(lbl_n)
        self.notes_edit = QTextEdit()
        self.notes_edit.setPlainText(self.notes_value)
        self.notes_edit.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Preferred)
        self.notes_edit.setMinimumHeight(64)
        self.notes_edit.setWordWrapMode(QTextOption.WrapMode.WordWrap)
        ...
        editor_css = """
            ...
            QTextEdit:focus { border: 1.5px solid #F59E0B; }
        """
        self.text_edit.setStyleSheet(editor_css)
        self.notes_edit.setStyleSheet(editor_css)
        ...
        body_layout.addWidget(self.save_btn, alignment=Qt.AlignmentFlag.AlignLeft)

        root.addWidget(self.body)

        # Toggle header expands/collapses
        self.header_btn.toggled.connect(lambda checked: self.body.setVisible(checked))

        # Start in view-only until global "Edit Questions" is toggled
        self.set_edit_mode(False)

    def set_edit_mode(self, editable: bool):
        """Enable/disable editing of the two fields and apply a subtle style change."""
        self._edit_mode = bool(editable)
        self.text_edit.setReadOnly(not self._edit_mode)
        self.notes_edit.setReadOnly(not self._edit_mode)
        ...
        """
        style = base_css if self._edit_mode else readonly_css
        self.text_edit.setStyleSheet(style)
        self.notes_edit.setStyleSheet(style)

    def expand(self, open_: bool = True):
        self.header_btn.setChecked(bool(open_))

    def save_question(self):
        """Save the edited question and notes."""
...
class WorkshopSetupTab(QWidget):
    def __init__(self):
        self.question_widgets = []  # holds CollapsibleQuestion widgets
        super().__init__()
        ...
        self.btn_edit_questions = QPushButton("✏️ Edit Questions")
        self.btn_edit_questions.setProperty("secondary", True)
        self.btn_edit_questions.setCheckable(True)
        self.btn_edit_questions.setToolTip("Toggle edit mode for all generated questions")
        self.btn_edit_questions.clicked.connect(self._toggle_edit_mode)
        rows.addWidget(self.btn_edit_questions)
        ...
        self.q_table.setStyleSheet("""
            ...
            QTableWidget::item:selected { background: #ffe8c2; }
...
# ---------------- internal helpers ----------------
    ...
    def _toggle_edit_mode(self):
        """Toggle edit mode for all CollapsibleQuestion cards and expand them when enabling."""
        enabled = False
        try:
            enabled = bool(self.btn_edit_questions.isChecked())
        except Exception:
            enabled = True  # if toggle doesn't exist, assume enabling

        if not getattr(self, "question_widgets", None):
            QMessageBox.information(self, "No Questions", "Generate questions first, then toggle edit mode.")
            try:
                self.btn_edit_questions.setChecked(False)
            except Exception:
                pass
            return
        ...
        for w in self.question_widgets:
            try:
                ...
                if enabled and hasattr(w, "expand"):
                    w.expand(True)
            except Exception:
                pass
        # Optional: brief status message if available
        try:
            sb = self.window().statusBar()
            if sb:
                sb.showMessage("Edit mode " + ("enabled" if enabled else "disabled"), 2500)
        except Exception:
            pass
    ...
    def _generate_questions(self):
        ...
        scroll = QScrollArea()
        scroll.setStyleSheet("QScrollArea { border: none; } QWidget { background: transparent; }")
        ...
        for group_label, qset in grouped_qs:
            ...
            header.setStyleSheet("font-weight:600; font-size:15px; margin:12px 0 6px 0;")
...
# === Generic collapsible section used by the Upload tab ==========================================
class CollapsibleSection(QWidget):
    def __init__(self, title: str, content: QWidget, parent=None):
        ...
        self.header_btn.toggled.connect(self.body.setVisible)
...
class UploadTab(QWidget):
    def __init__(self):
        ...
        # Title
        title = QLabel("Objective Assessment — Event-Driven")
        title.setStyleSheet("font-size: 22px; font-weight: 700; margin-bottom: 4px;")
        outer.addWidget(title)

        # Upload button (per spec)
        upload_row = QHBoxLayout()
        self.btn_upload = QPushButton("Upload Workshop Export JSON")
        self.btn_upload.setProperty("primary", True)
        self.btn_upload.setMinimumHeight(36)
        self.btn_upload.clicked.connect(self._upload_json)
        upload_row.addWidget(self.btn_upload, 0)
        upload_row.addStretch(1)
        outer.addLayout(upload_row)

        # Sections (hidden until a JSON is uploaded)
        self.sections_wrap = QWidget()
        self.sections_wrap.setVisible(False)
        sw = QVBoxLayout(self.sections_wrap)
        sw.setContentsMargins(0, 8, 0, 0)
        sw.setSpacing(12)

        # --- Section 1: Default / Manual Conversation (for quick tests)
        conv_widget = QWidget()
        cv = QVBoxLayout(conv_widget)
        cv.setSpacing(8)
        hint = QLabel("Optional: seed conversation text (used only when you press ‘Manually Ingest Below’).")
        hint.setStyleSheet("color:#6b7280;")
        cv.addWidget(hint)
        self.conv_text = QTextEdit()
        self.conv_text.setMinimumHeight(140)
        self.conv_text.setPlaceholderText(
            "CEO: We’re accelerating AI in R&D.\n"
            "CFO: Cost optimization remains a priority.\n"
            "CTO: Compliance automation is underway."
        )
        cv.addWidget(self.conv_text)
        btns = QHBoxLayout()
        self.btn_ingest = QPushButton("Manually Ingest Above Text")
        self.btn_clear_incoming = QPushButton("Clear Incoming File")
        self.btn_clear_incoming.setProperty("secondary", True)
        btns.addWidget(self.btn_ingest)
        btns.addWidget(self.btn_clear_incoming)
        btns.addStretch(1)
        cv.addLayout(btns)
        sec1 = CollapsibleSection("Default / Manual Conversation (for quick tests)", conv_widget)
        sw.addWidget(sec1)

        # --- Section 2: Control
        ctl_widget = QWidget()
        cl = QHBoxLayout(ctl_widget)
        cl.setSpacing(12)
        self.btn_check = QPushButton("Check for New Messages")
        self.btn_reval = QPushButton("Re-evaluate Now")
        self.btn_snapshot = QPushButton("Save Snapshot (_conv_N.json)")
        cl.addWidget(self.btn_check)
        cl.addWidget(self.btn_reval)
        cl.addWidget(self.btn_snapshot)
        cl.addStretch(1)
        self.incoming_label = QLabel("Incoming file: ./incoming/conversation.txt")
        self.incoming_label.setStyleSheet("color:#065f46;")
        cl.addWidget(self.incoming_label)
        sec2 = CollapsibleSection("Control", ctl_widget)
        sw.addWidget(sec2)

        # --- Section 3: Metrics
        metrics_widget = QWidget()
        ml = QVBoxLayout(metrics_widget)
        row = QHBoxLayout()
        def metric(title_text):
            box = QWidget()
            v = QVBoxLayout(box)
            lbl_t = QLabel(title_text)
            lbl_t.setStyleSheet("color:#4b5563;")
            lbl_v = QLabel("0")
            lbl_v.setStyleSheet("font-size:28px; font-weight:800;")
            v.addWidget(lbl_t)
            v.addWidget(lbl_v)
            return box
        row.addWidget(metric("Partial Chunks (this session)"))
        row.addWidget(metric("Snapshots Saved"))
        row.addWidget(metric("Objectives Tracked"))
        row.addWidget(metric("Role-Specific Follow-ups (distinct roles)"))
        ml.addLayout(row)
        tip = QLabel("Run an evaluation to see tables below.")
        tip.setStyleSheet("background:#eaf1ff; color:#1f2937; padding:10px; border-radius:8px;")
        ml.addWidget(tip)
        sec3 = CollapsibleSection("Metrics", metrics_widget)
        sw.addWidget(sec3)

        outer.addWidget(self.sections_wrap)

    def _upload_json(self):
        from PyQt6.QtWidgets import QFileDialog
        path, _ = QFileDialog.getOpenFileName(self, "Upload Workshop Export JSON", "", "JSON Files (*.json)")
        if not path:
            return
        self.selected_json_path = path
        # Reveal sections after a successful file selection
        self.sections_wrap.setVisible(True)
        # Update incoming file label with chosen file
        self.incoming_label.setText(f"Incoming file: {path}")
